...
type UtilityBillProjection = { periodStart: string; periodEnd: string; observedThrough: string; observedGridKwh: number; observedLoadKwh: number; averageDailyLoadKwh: number; averageDailyGridKwh?: number; averageHourlyGridKwh?: number; projectionMethod?: string; remainingDays: number; projectedFutureLoadKwh: number; projectedFutureSolarKwh: number; grossProjectedFutureSolarKwh?: number; solarOffsetFactor?: number; projectedFutureGridKwh: number; forecastDays: number; projectedGridKwh: number; projectedAmountClp: number; unitRateClp: number; archiveCoveragePct: number; coveredHours: number; lastDataAt: string | null; calculatedAt: string };
...
export default function EnelBillsSection({ deviceSn, siteLabel }: { deviceSn: string; siteLabel: string }) {
  ...
  const meterEntryRef = useRef<HTMLElement>(null);
  ...
  function goToMeterReading() {
    meterEntryRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    window.setTimeout(() => meterEntryRef.current?.querySelector<HTMLInputElement>('input')?.focus(), 450);
  }
  ...
  return <section className="enel-bills-section">
    <header className="enel-bills-heading"><div><small>Control real frente a Mi Solar · {siteLabel}</small><h2>Cuenta Enel</h2><p>Guarda siempre la cuenta y su monto final. La tarifa energética usa energía más traslado; deudas, intereses, descuentos e impuestos quedan registrados fuera del cálculo.</p></div><button type="button" className="primary-action" onClick={() => setOpen((value) => !value)}>{open ? <X/> : <FilePlus2/>}{open ? 'Cerrar' : 'Agregar cuenta'}</button></header>
    <button type="button" className="primary-action utility-meter-jump" disabled={!meterTracking} onClick={goToMeterReading}><Gauge/><span><b>Ingresar lectura medidor</b><small>{meterTracking ? `Período ${dateLabel(meterTracking.periodStart)} → ${dateLabel(meterTracking.periodEnd)}` : 'Se habilita cuando la última cuenta tiene lectura actual'}</small></span></button>
    ...
    {meterTracking ? <section className="panel utility-current-period" ref={meterEntryRef}><header><div><small>Mes en curso · dos proyecciones independientes</small><h3>{monthLabel(meterTracking.periodEnd)}</h3><p>{dateLabel(meterTracking.periodStart)} → {dateLabel(meterTracking.periodEnd)}</p></div></header><div className="utility-meter-entry"><label>Ingresar lectura del día <input type="text" inputMode="decimal" placeholder={meterValue(meterTracking.openingReadingKwh)} value={meterReading} onChange={(event)=>setMeterReading(event.target.value)}/></label><label>Nota opcional <input value={meterNote} maxLength={500} placeholder="Ej. lectura tomada por Carola" onChange={(event)=>setMeterNote(event.target.value)}/></label><button type="button" className="primary-action" disabled={meterSaving || !meterReading} onClick={()=>void saveMeterReading()}><Save/>{meterSaving?'Guardando…':'Guardar lectura'}</button></div><div className="utility-projection-methods">{projection ? <article><small>Proyección Mi Solar · red Estado 1</small><strong>{clp(projection.projectedAmountClp)}</strong><b>{kwh(projection.projectedGridKwh)} de red</b><p>{kwh(projection.observedGridKwh)} consumidos hasta ahora. Promedio lineal {kwh(projection.averageDailyGridKwh ?? projection.averageDailyLoadKwh)}/día, extrapolado hasta el cierre sin descontar nuevamente la producción solar.</p><em>Último dato: {projection.lastDataAt ? new Date(projection.lastDataAt).toLocaleString('es-CL',{timeZone:'America/Santiago'}) : 'sin muestras'}</em></article> : null}<article><small>Proyección por lecturas reales</small><strong>{meterTracking.projection.latestReadingKwh == null ? 'Ingresa una lectura' : clp(meterTracking.projection.projectedAmountClp)}</strong><b>{meterTracking.projection.latestReadingKwh == null ? `Inicio ${meterValue(meterTracking.openingReadingKwh)} kWh` : `${kwh(meterTracking.projection.projectedKwh)} proyectados`}</b><p>{kwh(meterTracking.projection.consumedKwh)} consumidos en {meterTracking.projection.elapsedDays} días · promedio {kwh(meterTracking.projection.averageDailyKwh)}/día.</p><em>Lectura inicial {meterValue(meterTracking.openingReadingKwh)}{meterTracking.projection.latestReadingKwh != null ? ` · última ${meterValue(meterTracking.projection.latestReadingKwh)} kWh` : ''}</em></article></div>{meterTracking.readings.length ? <section className="utility-meter-history"><h4>Historial de lecturas ({meterTracking.readings.length})</h4>{meterTracking.readings.map((reading,index)=>{const previous=meterTracking.readings[index+1];const previousValue=previous?.readingKwh??meterTracking.openingReadingKwh;const previousAt=previous?.readingAt??`${meterTracking.periodStart}T00:00:00`;const elapsedHours=Math.max(0,(Date.parse(reading.readingAt)-Date.parse(previousAt))/3_600_000);const consumed=Math.max(0,reading.readingKwh-previousValue);const hourly=elapsedHours>0?consumed/elapsedHours:0;return <div key={reading.id}><b>{meterValue(reading.readingKwh)} kWh</b><span>{new Date(reading.readingAt).toLocaleString('es-CL',{timeZone:'America/Santiago'})}<small>Consumo desde la anterior: {kwh(consumed)} · promedio {hourly.toLocaleString('es-CL',{maximumFractionDigits:3})} kWh/h · equivalente {kwh(hourly*24)}/24 h</small>{reading.notes ? <small>{reading.notes}</small>:null}</span></div>})}</section>:<p className="utility-meter-empty">Aún no hay lecturas del medidor en este período.</p>}</section> : projection ? <section className="panel utility-bill-projection"><header><div><small>Cuenta todavía no ingresada · estimación Mi Solar</small><h3>Proyección cuenta {monthLabel(projection.periodEnd)}</h3></div><strong>{clp(projection.projectedAmountClp)}</strong></header></section>:null}
