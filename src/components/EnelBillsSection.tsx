    ...
    {loading ? <div className="chart-loading">Consultando cuentas guardadas…</div> : loadError ? <section className="panel archive-read-error" role="alert"><FileImage/><div><strong>Las cuentas siguen respaldadas</strong><p>No se pudo consultar el archivo permanente. Mi Solar no reemplazará este error por una lista vacía.</p><small>{loadError}</small></div><button type="button" onClick={() => window.location.reload()}>Reintentar conexión</button></section> : bills.length ? <>
      <div className="enel-bill-list">{bills.map((bill, index) => {
        ...
        return <details className="panel enel-bill-card" key={bill.id}>
          <summary className="bill-period-summary"><div><small>{index === 0 ? 'Cuenta más reciente' : 'Cuenta guardada'} · {bill.source === 'photo-ai' ? '✨ IA' : 'Manual'}{duplicateMonths.has(bill.periodEnd.slice(0, 7)) ? <b className="bill-duplicate-badge">Duplicada</b> : null}</small><h3>{monthLabel(bill.periodEnd)}</h3><p>{dateLabel(bill.periodStart)} → {dateLabel(bill.periodEnd)} · {bill.periodDays} días</p>{bill.previousReading != null && bill.currentReading != null ? <small className="bill-summary-readings">Lectura anterior {meterValue(bill.previousReading)} · actual {meterValue(bill.currentReading)} kWh</small>:null}</div><div className="bill-period-kpis"><span><small>Consumo del período</small><strong>{bill.billedKwh > 0 ? kwh(bill.billedKwh) : 'Pendiente'}</strong><em className={bill.isEstimated ? 'estimated' : 'actual'}>{bill.isEstimated ? 'Estimado' : 'Real'}</em></span><span><small>Promedio diario</small><strong>{bill.averageDailyKwh == null ? 'Pendiente' : `${bill.averageDailyKwh.toLocaleString('es-CL', { maximumFractionDigits: 2 })} kWh/día`}</strong></span><span><small>Total a pagar</small><strong>{clp(bill.amountClp)}</strong></span><ChevronDown/></div></summary>
