...
type UtilityBill = {
  ...
  billedKwh: number; amountClp: number; effectiveRateClp: number | null; theoreticalGridKwh: number;
  ...
  serviceAddress?: string | null; fixedChargeClp?: number | null; energyChargeClp?: number | null; otherChargesClp?: number | null; taxesClp?: number | null;
  chargeItems?: ChargeItem[]; source?: string; aiConfidence?: number | null; documentCount?: number;
};
...
type ExtractedBill = {
  ...
  energyChargeClp: number | null; otherChargesClp: number | null; taxesClp: number | null; chargeItems: ChargeItem[]; confidence: number; warnings: string[];
};
...
export default function EnelBillsSection({ deviceSn, siteLabel }: { deviceSn: string; siteLabel: string }) {
  ...
  const calculatedRate = calculatedKwh > 0 && Number(draft.energyChargeClp || 0) > 0 ? Number(draft.energyChargeClp) / calculatedKwh : null;
  ...
  return <section className="enel-bills-section">
    ...
    {open ? <section className="panel enel-bill-form">
      ...
      <div className="enel-form-grid">
        ...
        <label>Cargo de energía del período <span>única base para calcular $/kWh</span><input type="number" min="0" step="1" inputMode="numeric" value={draft.energyChargeClp} onChange={(event) => setField('energyChargeClp', event.target.value)}/></label>
      ...
      </div>{aiExtraction?.chargeItems?.length ? <div className="bill-charge-breakdown"><header><strong>Desglose detectado</strong><small>Solo las filas verdes forman el valor energético</small></header>{aiExtraction.chargeItems.map((item, index) => <div className={item.includedInEnergyRate ? 'included' : ''} key={`${item.label}-${index}`}><span>{item.label}<small>{item.includedInEnergyRate ? 'Incluido en $/kWh' : 'Guardado, fuera del cálculo'}</small></span><b>{clp(item.amountClp)}</b></div>)}</div> : null}{aiExtraction?.warnings?.length ? <ul className="bill-ai-warnings">{aiExtraction.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul> : null}</details>
      <div className="bill-rate-formula"><b>Fórmula energética</b><span>{clp(Number(draft.energyChargeClp || 0))} de energía ÷ {kwh(calculatedKwh)} = <strong>{calculatedRate == null ? 'Pendiente' : `${clp(calculatedRate)} / kWh`}</strong></span><small>El monto total, cargo fijo, impuestos, descuentos, deudas, intereses, repactaciones y otros cobros quedan fuera de esta división.</small></div>
