...
export default function EnergyFlow({data,history}:{data:Realtime;history:HistoryRow[]}){
 const p1=pvPower(data,1),p2=pvPower(data,2),solar=p1+p2,grid=gridPower(data),load=loadPower(data),charge=batteryChargePower(data),discharge=batteryDischargePower(data),soc=batterySoc(data),pvCount=detectPvCount(data,history);
 ...
 <div className={`flow-stage pv-${pvCount}`}>
   ...
   <Node className="grid" title="Red eléctrica" value={watts(Math.abs(grid))} sub={gridReverse?'Exportando':'Importando'} icon={RadioTower}/>
