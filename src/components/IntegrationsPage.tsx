...
type AppSession={authenticated:boolean;authRequired:boolean;user?:{displayName:string};access:{role:string;permissions:string[]}};
...
export default function IntegrationsPage({siteLabel,siteId}:{siteLabel:string;siteId?:number}){
  ...
  const [appSession,setAppSession]=useState<AppSession|null>(null),[providerSite,setProviderSite]=useState<ProviderSite|null>(null),[providerBusy,setProviderBusy]=useState(''),[providerMessage,setProviderMessage]=useState('');
  const [adminLogin,setAdminLogin]=useState({username:'',password:''}),[editing,setEditing]=useState<ProviderName|null>(null),[credentials,setCredentials]=useState({username:'',password:''});
  ...
  async function loadProviders(){
    ...
    if(!session.authenticated||!siteId){setProviderSite(null);return}
    const data=await api<{sites:ProviderSite[]}>('provider-accounts');setProviderSite(data.sites.find(site=>site.id===siteId)||null);
  }
  useEffect(()=>{void loadProviders().catch(cause=>setProviderMessage(cause instanceof Error?cause.message:'No se pudieron consultar las credenciales.'))},[siteId]);
  ...
  async function saveCredentials(event:React.FormEvent){event.preventDefault();if(!editing||!siteId)return;setProviderBusy(editing);setProviderMessage('');try{await api(`sites/${siteId}/providers/${editing}/credentials`,{method:'PUT',body:JSON.stringify(credentials)});setCredentials({username:'',password:''});setEditing(null);await loadProviders();setProviderMessage('Credenciales reemplazadas de forma segura. La contraseña anterior no puede recuperarse.')}catch(cause){setProviderMessage(cause instanceof Error?cause.message:'No se pudieron guardar las credenciales.')}finally{setProviderBusy('')}}
  async function providerAction(provider:ProviderName,action:'test'|'sync'){if(!siteId)return;setProviderBusy(`${provider}-${action}`);setProviderMessage('');try{await api(`sites/${siteId}/providers/${provider}/${action}`,{method:'POST'});await loadProviders();setProviderMessage(action==='test'?'Conexión comprobada correctamente.':'Sincronización terminada y guardada en el histórico propio de MiSolar.')}catch(cause){setProviderMessage(cause instanceof Error?cause.message:'No se pudo completar la operación.')}finally{setProviderBusy('')}}
  async function removeCredentials(provider:ProviderName){if(!siteId||!window.confirm(`¿Confirmas desconectar ${PROVIDER_LABEL[provider]} y eliminar sus credenciales cifradas? Los históricos de MiSolar se conservarán.`))return;setProviderBusy(`${provider}-delete`);try{await api(`sites/${siteId}/providers/${provider}/credentials`,{method:'DELETE'});await loadProviders();setProviderMessage('Proveedor desconectado. El histórico fue conservado.')}catch(cause){setProviderMessage(cause instanceof Error?cause.message:'No se pudo desconectar.')}finally{setProviderBusy('')}}
  ...
  return <section className="settings-page"><header className="page-heading"><div><small>Conexiones · {siteLabel}</small><h1>Credenciales e integraciones</h1><p>Administra cada proveedor desde el servidor; ninguna contraseña se guarda ni se recupera desde este navegador.</p></div><button className="refresh-button" onClick={()=>{void loadProviders();void loadDevices()}} disabled={busy||Boolean(providerBusy)}><RefreshCw className={busy||providerBusy?'spin':''}/><span>Actualizar</span></button></header>{error&&<div className="data-warning-banner">{error}</div>}{providerMessage&&<div className="data-warning-banner">{providerMessage}</div>}
    {!appSession?.authenticated?<section className="panel provider-admin-gate"><ShieldCheck/><div><h2>Área protegida</h2><p>La interfaz general puede abrirse sin login cuando se habilite el modo temporal, pero las credenciales, usuarios y finanzas siempre requieren autorización.</p><form onSubmit={loginAdmin}><input autoComplete="username" placeholder="Usuario MiSolar" value={adminLogin.username} onChange={event=>setAdminLogin(value=>({...value,username:event.target.value}))}/><input autoComplete="current-password" type="password" placeholder="Contraseña MiSolar" value={adminLogin.password} onChange={event=>setAdminLogin(value=>({...value,password:event.target.value}))}/><button className="primary-action" disabled={providerBusy==='login'}>{providerBusy==='login'?'Comprobando…':'Administrar integraciones'}</button></form></div></section>:<section className="provider-account-grid">{(providerSite?.providers||[]).map(account=><article className="panel provider-account-card" key={account.provider}><header><span className="provider-logo"><Wifi/></span><div><small>Proveedor</small><h2>{PROVIDER_LABEL[account.provider]}</h2></div><b className={`provider-status status-${account.status}`}>{STATUS_LABEL[account.status]||account.status}</b></header>{account.provider==='watchpower'&&<div className="provider-readonly-banner">WatchPower: solo lectura</div>}<dl><div><dt>Usuario configurado</dt><dd>{account.usernameMasked||'—'}</dd></div><div><dt>Contraseña</dt><dd>{account.passwordConfigured?'••••••••••••':'Sin configurar'}</dd></div><div><dt>Última conexión exitosa</dt><dd>{dateTime(account.lastSuccessAt)}</dd></div><div><dt>Último intento</dt><dd>{dateTime(account.lastAttemptAt)}</dd></div><div><dt>Dispositivo vinculado</dt><dd>{account.devices[0]?.alias||account.devices[0]?.model||'Pendiente'}</dd></div><div><dt>Número de serie</dt><dd>{account.devices[0]?.serialMasked||'—'}</dd></div><div><dt>Última lectura</dt><dd>{dateTime(account.devices[0]?.lastReadingAt)}</dd></div></dl><div className="provider-actions"><button onClick={()=>providerAction(account.provider,'test')} disabled={Boolean(providerBusy)}>Probar conexión</button><button onClick={()=>{setEditing(account.provider);setCredentials({username:'',password:''})}} disabled={Boolean(providerBusy)}>Actualizar credenciales</button><button onClick={()=>providerAction(account.provider,'sync')} disabled={Boolean(providerBusy)||!account.enabled}>Sincronizar ahora</button><button className="danger-action" onClick={()=>removeCredentials(account.provider)} disabled={Boolean(providerBusy)||!account.passwordConfigured}><Unplug size={16}/>Desconectar</button></div>{editing===account.provider&&<form className="provider-credentials-form" onSubmit={saveCredentials}><h3>Reemplazar credenciales</h3><p>La contraseña guardada nunca se muestra. Al guardar, se sustituye por la nueva versión cifrada.</p><input autoComplete="off" placeholder="Usuario" value={credentials.username} onChange={event=>setCredentials(value=>({...value,username:event.target.value}))}/><input autoComplete="new-password" type="password" placeholder="Nueva contraseña" value={credentials.password} onChange={event=>setCredentials(value=>({...value,password:event.target.value}))}/><div><button type="button" onClick={()=>{setEditing(null);setCredentials({username:'',password:''})}}>Cancelar</button><button className="primary-action">Guardar cifrada</button></div></form>}</article>)}</section>}
