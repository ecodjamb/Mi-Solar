...
export default function IntegrationsPage({siteLabel,siteId}:{siteLabel:string;siteId?:number}){
  ...
  async function saveCredentials(event:React.FormEvent){event.preventDefault();if(!editing)return;const key=`${editing.siteId}-${editing.provider}`;setProviderBusy(key);setProviderMessage('');try{await api(`sites/${editing.siteId}/providers/${editing.provider}/credentials`,{method:'PUT',body:JSON.stringify(credentials)});setCredentials({username:'',password:''});setEditing(null);await loadProviders();setProviderMessage('Credenciales guardadas de forma cifrada. La contraseña nunca se mostrará ni podrá recuperarse.')}catch(cause){setProviderMessage(cause instanceof Error?cause.message:'No se pudieron guardar las credenciales.')}finally{setProviderBusy('')}}
