...
import { chileSiteRangeApiRange,dailyEnergy,formatSiteDate,gridPower,loadPower,parseApiTime,pvPower,siteRangeUtc,siteWeekDateRange,kwh } from '../utils/energy';
...
export default function HistoryExplorer({deviceSn,siteLabel}:{deviceSn:string;siteLabel:string}){
  ...
  const option=useMemo(()=>({tooltip:{trigger:'axis',confine:true},legend:{textStyle:{color:'#a9bdc3'}},grid:{left:48,right:18,top:54,bottom:42,containLabel:true},xAxis:{type:'category',data:parsed.map(item=>item.time.toLocaleString('es-CL',{timeZone:'America/Santiago',day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})),axisLabel:{color:'#789099',hideOverlap:true}},yAxis:{type:'value',name:'W',axisLabel:{color:'#789099'},splitLine:{lineStyle:{color:'rgba(110,150,160,.12)'}}},series:[{name:'Solar',type:'line',showSymbol:false,data:parsed.map(item=>pvPower(item.row,1)+pvPower(item.row,2)),lineStyle:{color:'#efbd34'},itemStyle:{color:'#efbd34'}},{name:'Consumo',type:'line',showSymbol:false,data:parsed.map(item=>loadPower(item.row)),lineStyle:{color:'#a96fff'},itemStyle:{color:'#a96fff'}},{name:'Red',type:'line',showSymbol:false,data:parsed.map(item=>gridPower(item.row)),lineStyle:{color:'#4f9fff'},itemStyle:{color:'#4f9fff'}}]}),[parsed]);
