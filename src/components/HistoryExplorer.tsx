...
import { chileSiteRangeApiRange,dailyEnergy,effectiveGridPower,formatSiteDate,loadPower,parseApiTime,pvPower,siteRangeUtc,siteWeekDateRange,kwh,solarSystemToLoadPower } from '../utils/energy';
...
export default function HistoryExplorer({deviceSn,siteLabel,gridLabel='Red activa'}:{deviceSn:string;siteLabel:string;gridLabel?:string}){
  ...
  async function search(){
    ...
    try{const utc=siteRangeUtc(range.start,range.end);const resolution=mode==='year'?'day':'hour';const stored=await api<{list:HistoryRow[]}>(`devices/${deviceSn}/archive-series?start=${encodeURIComponent(utc.start)}&end=${encodeURIComponent(utc.end)}&resolution=${resolution}`);if(stored.list?.length){setRows(stored.list);setSource('Archivo permanente Mi Solar');setMessage(`${stored.list.length} períodos consolidados recuperados.`);return}const apiRange=chileSiteRangeApiRange(range.start,range.end);const fresh=await api<{list:HistoryRow[]}>(`devices/${deviceSn}/history?start=${encodeURIComponent(apiRange.start)}&end=${encodeURIComponent(apiRange.end)}&maxPages=50`);setRows(fresh.list||[]);setSource('Origen · nuevos datos respaldados');setMessage(`${fresh.list?.length||0} muestras nuevas encontradas.`)}
    catch(error){setRows([]);setSource('');setMessage(error instanceof Error?error.message:'No fue posible consultar el período.')}finally{setLoading(false)}
  }
  const dateType=mode==='month'?'month':mode==='year'?'number':'date';
  return <section className="panel history-explorer"><header><div><small>Histórico permanente · {siteLabel}</small><h2><CalendarSearch size={22}/> Día · semana · mes · año</h2></div>{source&&<span><Database size={14}/>{source}</span>}</header><div className="history-search-form"><label>Período<select value={mode} onChange={event=>setMode(event.target.value as Mode)}><option value="day">Día</option><option value="week">Semana</option><option value="month">Mes</option><option value="year">Año</option><option value="range">Rango personalizado</option></select></label><label>{mode==='month'?'Mes':mode==='year'?'Año':'Fecha'}<input type={dateType} min={mode==='year'?'2020':undefined} max={mode==='year'?'2100':undefined} value={mode==='month'?date.slice(0,7):mode==='year'?date.slice(0,4):date} onChange={event=>setDate(mode==='month'?`${event.target.value}-01`:mode==='year'?`${event.target.value}-01-01`:event.target.value)}/></label>{mode==='range'&&<label>Hasta<input type="date" value={endDate} min={date} onChange={event=>setEndDate(event.target.value)}/></label>}<button type="button" onClick={search} disabled={loading}>{loading?'Buscando…':'Consultar período'}</button></div><p className="history-search-message">{message}</p>{rows.length>1&&<><div className="history-result-kpis"><span><small>Períodos</small><strong>{rows.length.toLocaleString('es-CL')}</strong></span><span><small>Solar</small><strong>{kwh(energy.solar)}</strong></span><span><small>Consumo</small><strong>{kwh(energy.load)}</strong></span><span><small>{gridLabel}</small><strong>{kwh(energy.gridImport)}</strong></span></div><EChart option={option}/></>}</section>;
}
