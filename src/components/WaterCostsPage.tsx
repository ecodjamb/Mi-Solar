...
type WaterBill = {
  ...
  billedM3: number; averageDailyM3: number | null; consumptionStatus: 'actual'|'estimated'|'pending'|'unavailable'; isEstimated: boolean; estimateMethod?: string | null; classificationReason?: string | null;
...
export default function WaterCostsPage({ deviceSn, siteLabel }: { deviceSn: string; siteLabel: string }) {
  ...
  const chartOption = useMemo(() => ({
    tooltip: { trigger: 'axis', confine: true, backgroundColor: '#071b23', borderColor: '#3c7182', textStyle: { color: '#edf9fb' }, formatter: (params: unknown) => { const items = Array.isArray(params) ? params as Array<{ dataIndex?: number }> : []; const bill = chartBills[Number(items[0]?.dataIndex || 0)]; return bill ? `<b>${monthLabel(billMonth(bill))}</b><br/><span style="color:${bill.isEstimated ? '#f3a847' : '#38bdf8'}">●</span> ${bill.isEstimated ? 'Estimado · sin lectura completa' : 'Real · con lecturas fechadas'}: <b>${m3(bill.billedM3)}</b><br/>Promedio diario: ${m3(bill.averageDailyM3, 2)}<br/>Cuenta: <b>${clp(bill.amountClp)}</b>` : ''; } },
    legend: { top: 4, left: 'center', textStyle: { color: '#a9bdc3' }, itemWidth: 15, itemHeight: 9 }, grid: { left: 34, right: 38, top: 70, bottom: 48, containLabel: true },
    xAxis: { type: 'category', data: chartBills.map((bill) => monthLabel(billMonth(bill)).replace(' de ', ' ')), axisLabel: { color: '#9db0b6', hideOverlap: true, rotate: chartBills.length > 8 ? 26 : 0 }, axisLine: { lineStyle: { color: '#31525d' } } },
    ...
    series: [
      { name: 'Lectura real', type: 'bar', stack: 'water', barMaxWidth: 44, data: chartBills.map((bill) => bill.isEstimated ? null : bill.billedM3), itemStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: '#66dcff' }, { offset: 1, color: '#1689c7' }] }, borderRadius: [8, 8, 2, 2] }, label: { show: true, position: 'top', color: '#c9f3ff', fontSize: 9, formatter: ({ value }: { value: number }) => value == null ? '' : Number(value).toLocaleString('es-CL', { maximumFractionDigits: 1 }) } },
      { name: 'Consumo estimado', type: 'bar', stack: 'water', barMaxWidth: 44, data: chartBills.map((bill) => bill.isEstimated ? bill.billedM3 : null), itemStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: '#ffd27a' }, { offset: 1, color: '#e38a20' }] }, borderRadius: [8, 8, 2, 2] }, label: { show: true, position: 'top', color: '#ffe3ad', fontSize: 9, formatter: ({ value }: { value: number }) => value == null ? '' : Number(value).toLocaleString('es-CL', { maximumFractionDigits: 1 }) } },
      { name: 'Monto de la cuenta', type: 'line', yAxisIndex: 1, data: chartBills.map((bill) => bill.amountClp), symbolSize: 8, smooth: 0.25, lineStyle: { color: '#72e0a6', width: 2.5 }, itemStyle: { color: '#72e0a6', borderColor: '#0a222b', borderWidth: 2 }, areaStyle: { color: 'rgba(75,210,139,.07)' } }
    ]
  }), [chartBills]);
  ...
  async function chooseBillFiles(files: FileList | null) {
    ...
    try {
      ...
      setBillImages(next); setBillAi(null);
  ...
  async function importBillAutomatically(images: WaterImage[]) {
    setBillBusy(true); setError(''); setMessage('La IA está leyendo y guardando la boleta automáticamente…');
    try {
      const result = await api<{ extracted: WaterBillExtract; model: string }>(`devices/${deviceSn}/water-bills/extract`, { method: 'POST', body: JSON.stringify({ images }) });
      const value = result.extracted;
      const draft = draftFromExtract(defaultBillDraft(), value);
      setBillAi(value); setBillModel(result.model); setBillDraft(draft);
      const saved = await api<{ bill: WaterBill & { documentWarnings?: string[] } }>(`devices/${deviceSn}/water-bills`, { method: 'POST', body: JSON.stringify({ ...draft, chargeItems: value.chargeItems || [], images, aiExtraction: value, aiConfidence: value.confidence, aiModel: result.model }) });
      ...
      setMessage(`Cuenta de ${monthLabel(billMonth(saved.bill))} guardada automáticamente como ${saved.bill.isEstimated ? 'consumo estimado' : 'lectura real'}.${warning ? ` ${warning}` : ''}`);
      await reload();
    } catch (cause) {
      setError(cause instanceof Error ? `${cause.message} Puedes abrir “Revisión manual” y guardarla igualmente.` : 'No fue posible procesar la boleta.');
  ...
  async function closePeriod() {
    if (!dashboard?.period) return;
    setReadingBusy(true); setError('');
    try { await api(`devices/${deviceSn}/water-periods/close`, { method: 'POST', body: JSON.stringify({ periodId: dashboard.period.id, closingReadingM3: Number(closeValue), readingAt: new Date().toISOString() }) }); setCloseValue(''); setMessage('Mes cerrado: consumo consolidado en el historial y nuevo período abierto automáticamente.'); await reload(); }
  ...
  return <section className="water-costs-page">
    <header className="water-hero"><div className="water-hero-icon"><Droplets/></div><div><small>Control de Aguas Andinas · {siteLabel}</small><h1>Costos y consumo de agua</h1><p>Boletas, lecturas del medidor y proyección del mes en un solo lugar.</p></div><button type="button" onClick={() => setBillOpen((value) => !value)}><FilePlus2/> Subir boleta</button></header>
    {message ? <p className="water-message"><CheckCircle2/> {message}</p> : null}
    {error ? <p className="water-error">{error}</p> : null}

    <section className="panel water-chart-card"><header><div><small>Lo primero: historial respaldado</small><h2>Agua consumida y total pagado</h2><p>Azul: lectura real verificada · naranja: estimación sin lectura completa.</p></div><nav aria-label="Período visible"><button className={chartFilter === '6m' ? 'active' : ''} onClick={() => setChartFilter('6m')}>6 meses</button><button className={chartFilter === '12m' ? 'active' : ''} onClick={() => setChartFilter('12m')}>12 meses</button>{years.map((year) => <button className={chartFilter === year ? 'active' : ''} onClick={() => setChartFilter(year)} key={year}>{year}</button>)}</nav></header>{chartBills.length ? <EChart option={chartOption} className="water-chart"/> : <div className="water-empty-chart"><Droplets/><b>Aún no hay cuentas guardadas</b><span>Sube la primera boleta para iniciar el historial.</span></div>}<footer><span><b>{m3(chartBills.reduce((sum, bill) => sum + bill.billedM3, 0))}</b> consumo visible</span><span><b>{clp(chartBills.reduce((sum, bill) => sum + bill.amountClp, 0))}</b> total pagado</span><span><b>{chartBills.filter((bill) => !bill.isEstimated).length} reales · {chartBills.filter((bill) => bill.isEstimated).length} estimadas</b> clasificación automática</span></footer></section>
