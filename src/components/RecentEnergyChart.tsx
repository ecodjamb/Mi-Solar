...
export default function RecentEnergyChart({rows,siteLabel}:{rows:HistoryRow[];siteLabel:string}){
 ...
 const option=useMemo(()=>({
  ...
  series:[
   {name:'Consumo total casa',type:'line',smooth:true,showSymbol:false,data:recent.map(item=>loadPower(item.row)),lineStyle:{width:3,color:'#a96fff'},areaStyle:{opacity:.06,color:'#a96fff'}},
   {name:'Generación PV1 + PV2',type:'line',smooth:true,showSymbol:false,data:recent.map(item=>pvPower(item.row,1)+pvPower(item.row,2)),lineStyle:{width:3,color:'#efbd34'},areaStyle:{opacity:.05,color:'#efbd34'}},
   {name:'Batería (+ entrega / − carga)',type:'line',smooth:true,showSymbol:false,data:recent.map(item=>batteryDischargePower(item.row)-batteryChargePower(item.row)),lineStyle:{width:2,color:'#4bd98a'}},
